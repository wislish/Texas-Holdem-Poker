...
def compare_hands(h1, h2):
    ...
    if one[0] == two[0]:
        ...
        if one[0] == "STRAIGHT FLUSH":
            ...
            if com == "TIE":
                return "none", convert_toletters(one[1]), convert_toletters(two[1])
            elif com == "RIGHT":
                return "right", two[0], convert_toletters(two[1])
            else:
...
def getwinner(a, b):
    ...
    if result[0] == "right":
        ...
        win_hand = a
    biggerone = convert_toletters(win_hand)
    return biggerone
